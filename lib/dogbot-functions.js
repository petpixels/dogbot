...
module.exports = {
	reply: function(dog,classifier,sentiment,channel,input,user) {
		...
		console.log(classifier)
		var db_reply = this.build_reply(dog,classifier,sentiment,channel,input,user)
		...
		if (db_reply) {
			...
			var logString = user // + "\t" + dogSentiment.score

			if (lowerCaseReply.includes("woo")) {
				//console.log("logging good")
				this.train(dog,user,1)
			...
			var lcMsg = input.toLowerCase().replace(/[.,\/#!\\?$%\^&\*;:{}=\-_`~()]/g,"").replace(/\n/g," ")
			var commands = [
				"good dog",
				"good boy",
				"good girl",
	...
	build_reply: function(dog,classifier,sentiment,channel,input,user) {
		console.log(channel)
		console.log(input)
		console.log(user)
